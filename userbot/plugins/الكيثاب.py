...
@icssbot.on(admin_cmd(pattern="جيتهاب (.*)", outgoing=True))
@icssbot.on(sudo_cmd(pattern="جيتهاب (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@icssbot.on(admin_cmd(pattern="رفع جيتهاب$", outgoing=True))
@icssbot.on(sudo_cmd(pattern="رفع جيتهاب$", allow_sudo=True))
...
CMD_HELP.update(
    {
        "جيتهاب": "**Plugin : **`جيتهاب`\
        \n\n**Syntax : **`.جيتهاب + يوزر`\
        \n**Function : ** __Get information about an user on GitHub of given username__\
        \n\n**Syntax : **`.رفع جيتهاب` بالرد ع ملف بايثون لرفعـه ع جيتهاب\
