...
def pack_nick(username, pack, is_anim):
    if Config.CUSTOM_STICKER_PACKNAME:
        if is_anim:
            packnick = f"{Config.CUSTOM_STICKER_PACKNAME} حقوق {pack} (Animated)"
        else:
            packnick = f"{Config.CUSTOM_STICKER_PACKNAME} حقوق {pack}"
    else:
        if is_anim:
            packnick = f"@{username} حقوق {pack} (Animated)"
        else:
            packnick = f"@{username} حقوق {pack}"
    return packnick
...
async def kang(args):
    ...
    if photo:
        ...
        packnick = (
            f"{telename} حقوق {pack}"
            if telename
            else f"@{user.username} حقوق {pack}"
        )
        cmd = "/newpack"
        file = io.BytesIO()
...
async def pack_kang(event):
    ...
    for message in reqd_sticker_set.documents:
        if "image" in message.mime_type.split("/"):
            await edit_or_reply(
                icssevent,
                f"**- جـاري استنساخ حزمه الملصقـات بحقـوقك ↫ العدد : {kangst}/{noofst} .**",
            )
            photo = io.BytesIO()
    ...
    result = "**- تم نسخ الحزمه بحقوقك بنجاح ☑️.**\n"
    for i in range(len(blablapacks)):
        result += f"  •  [حزمه {blablapacknames[i]}](t.me/addstickers/{blablapacks[i]})"
