...
async def cmd_list(event):
    ...
    if input_str == "text":
        string = (
            "المجمـوع {count} أمر تم ايجاد {plugincount} اضافـه لبوت ايجيثون\n\n"
        )
        catcount = 0
        plugincount = 0
        for i in sorted(CMD_LIST):
            plugincount += 1
            string += f"᥀︙ {plugincount}) الاوامر الموجوده في هذه الاضافه " + i + " هي: \n"
        ...
        if len(string) > 4095:
            ...
            reply_text = f"**᥀︙ جميع الاوامر لبوت ايجيثون تستطيـع الحصول عليها [هـنا]({url})**"
...
•  **• الامر **`.مساعده/.مساعده + اسم الاضافه`
•  **• الشرح **__If you just type .help then shows you help menu, if plugin name is given then shows you only commands in thst plugin and if you use `.help text` then shows you all commands in your userbot__

•  **• الامر **`.شرح/.شرح + اسم الاضافه`
•  **• الشرح **__To get details/information/usage of that plugin__

•  **• الامر **`.المراكز`
•  **• الشرح **__Shows your dc id and dc ids list__

•  **• الامر **`.انلاين (تفعيل|تعطيل)`
•  **• الشرح **__Sets help menu either in inline or text format__"""
    }
)
