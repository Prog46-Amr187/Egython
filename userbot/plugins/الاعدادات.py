...
async def _(event):
    ...
    stdout, stderr = await process.communicate()
    o = stdout.decode()
    OUTPUT = f"**᥀︙ [• 𝐒𝐨𝐮𝐫𝐜𝐞 - 𝐄𝐠𝐲𝐭𝐡𝐨𝐧.](tg://need_update_for_some_feature/) الاضافات:**\n{o}"
...
async def _(event):
    ...
    o = stdout.decode()
    OUTPUT = (
        f"**[• 𝐒𝐨𝐮𝐫𝐜𝐞 - 𝐄𝐠𝐲𝐭𝐡𝐨𝐧.](tg://need_update_for_some_feature/) Environment Module:**\n\n\n{o}"
    )
    if len(OUTPUT) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            out_file.name = "env.text"
...
async def _(event):
    ...
    stdout, stderr = await process.communicate()
    o = stdout.decode()
    OUTPUT = f"**[• 𝐒𝐨𝐮𝐫𝐜𝐞 - 𝐄𝐠𝐲𝐭𝐡𝐨𝐧.](tg://need_update_for_some_feature/) , تم حساب سرعة السيرفر:**\n{o}"
    if len(OUTPUT) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            out_file.name = "env.text"
            await event.client.send_file(
...
CMD_HELP.update(
    ...
    \n\n**◄  الامـر ⦂ **`.الاضافات`\
    \n**الشـرح •• **يعرض لك قائمة الاضافات الموجودة في البوت\
    \n\n**◄  الامـر ⦂ **`.تاريخ`\
