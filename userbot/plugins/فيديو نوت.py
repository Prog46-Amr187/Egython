...
async def _(event):
    ...
    if not reply_message:
        await edit_or_reply(event, "**```بالرد علي الفيديو```**")
        return
    chat = "@SYRAPK_CONVERTBOT"
    catevent = await edit_or_reply(event, "**جاري التحويل🎆**")
...
CMD_HELP.update(
    ...
    \n\n**• الامر **`.نوت` بالرد على الفيـديـو\
    \n**• الشرح **تحـويل الفيديـو الـى فيديـو نـوت"
    }
)
