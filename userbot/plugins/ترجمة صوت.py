...
@icssbot.on(admin_cmd(pattern="ترجمة صوت (.*)"))
@icssbot.on(sudo_cmd(pattern="ترجمة صوت (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    start = datetime.now()
...
CMD_HELP.update(
    {
        "ترجمة صوت": "**اسم الاضافـه : **`ترجمة صوت`\
    \n\n**• الأمر** `.ترجمة صوت en` بالرد على الرسالـه الصوتيـه\
    \n**• الشرح **ترجمـة صوت الـى نـص"
    }
)
