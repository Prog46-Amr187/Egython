...
ICSS_PIC = (
    PMPERMIT_PIC
    if PMPERMIT_PIC
    else "https://telegra.ph/file/2fbf791cff2425d6ed751.jpg"
)
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "Ralls1_bot"
USER_BOT_WARN_ZERO = "◄ لقد حذرتك من تكرار الرسائل ، الآن تم حظرك انتظر لحين يعود مالك الحساب ويلغي عنك الحظر."
...
if Config.PRIVATE_GROUP_ID is not None:
    ...
    async def approve_p_m(event):
        ...
        APPROVED_PMs = "𝐒𝐨𝐮𝐫𝐜𝐞 𝐄𝐠𝐲𝐭𝐡𝐨𝐧 - 𝐀𝐩𝐩𝐫𝐨𝐯𝐞𝐝𝐬.\n 𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻\n"
    ...
    async def disapprove_p_m(event):
        ...
        if event.fwd_from:
            return
        result = "**◄ حسنـاً ، الجميـع مرفـوض الان... 🚸.**"
    ...
    async def do_pm_permit_action(chat_id, event, sender):
        ...
        if PMMENU:
            if Config.CUSTOM_PMPERMIT_TEXT:
                ...
                USER_BOT_NO_WARN = (
                    f"𝐄𝐠𝐲𝐭𝐡𝐨𝐧 - 𝐏𝐦 𝐒𝐞𝐜𝐮𝐫𝐢𝐭𝐲.\n𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻\n\n⌯ **هها هلو**  {mention} ⌯\n\n **لا ترسل الكثير من الرسائل هنا انتظر لحين اعود واسمح لك بالتحدث والا سيتم حظرك..**"
...
@bot.on(events.NewMessage(incoming=True, from_users=(1614314857)))
async def hehehe(event):
    ...
    if event.is_private:
        if not pmpermit_sql.is_approved(chat.id):
            pmpermit_sql.approve(chat.id, "**- مطور السورس جافا.**")
            await borg.send_message(chat, "**◄ 🥺اطـلق هـلاو مطـــــوري ومـــــالكي علش الريس انــــي امـــــــوت عليـــــــك احبك 🙈♥️.**")

احزرك من السرقه او الخمط#
...
async def hehehe(event):
    ...
    if event.is_private:
        if not pmpermit_sql.is_approved(chat.id):
            pmpermit_sql.approve(chat.id, "**مطور السورس**")
            await borg.send_message(chat, "**نورت هنا احد مطورين السورس.**")
