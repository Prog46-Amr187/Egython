...
async def _(event):
    ...
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID, "#اعاده_التشغيل \n" "◄  بوت ايجي ثون في وضع اعاده التشغيل انتظر"
        )
    await edit_or_reply(
        event,
        f"**᥀︙ اهلا عزيزي** - {mention}\n"
        f"**يتم الان اعـادة تشغيـل بـوت ايجي ثون يستغـرق الامر 1-2 دقيقه ⏳.**",
    )
...
async def _(event):
    ...
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID, "#اطفاء \n" "بوت ايجي ثون في وضع الاطفاء"
        )
    await edit_or_reply(
        event, "**جارٍ إيقاف تشغيل بوت ايجي ثون الآن ... شغِّلني يدويًا لاحقًا**"
    )
