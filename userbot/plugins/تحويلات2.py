...
async def _(event):
    ...
    if mediatype and mediatype != "video":
        return await edit_delete(event, "**◄ بالرد علي الفديو للتحويل لمتحركه.**")
    args = event.pattern_match.group(1)
    if not args:
        args = 2.0
    else:
        try:
            ...
            args = 2.0
    catevent = await edit_or_reply(event, "**◄ جاري تحويل الفديو الي متحركه انتظر قليلا 🎞.**")
...
CMD_HELP.update(
    ...
    \n\n**• الأمر **`.دائري` بالرد ؏ صـوره او ملـصق\
    \n**• الشرح**تحويل الصـوره او الملـصق الـى شكـل دائـري\
    \n\n**• الأمر **`.لمتحركه` بالـرد ع صـوره\
    \n**• الشرح**تحويل صوره لــ متحـركـه جـاهزه\
    \n\n**• الأمر** `.لمتحرك` بالـرد ؏ ملصق متحـرك\
    \n**• الشرح** تحويل ملصق متحرك لــ متحـركة\
    \n\n**• الأمر** `.متحرك` بالـرد ع فيديـو\
    \n**• الشرح** لتحـويل الفيديـو الـى متحـركـه\
