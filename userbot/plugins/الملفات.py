# file summary plugin for Egython  by @java edit by @Egython
...
@borg.on(admin_cmd(pattern="احصائيات الجروب ?(.*)", outgoing=True))
@borg.on(sudo_cmd(pattern="احصائيات الجروب ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    ...
    catevent = await edit_or_reply(
        event,
        f"<code>جارِ ... عد الميديا وحجم كل ملف لجروب </code><b>{link}</b>\n<code>قد يستغرق هذا بعض الوقت يعتمد أيضًا على عدد رسائل الجروب</code>",
        parse_mode="HTML",
    )
    media_dict = {
...
async def _(event):
    ...
    catevent = await edit_or_reply(
        event,
        f"<code>جارِ ... عد الملفات وحجم كل ملف بواسطة </code>{_format.htmlmentionuser(userdata.first_name,userdata.id)}<code> في جروب </code><b>{link}</b>\n<code>قد يستغرق هذا بعض الوقت يعتمد ايضا على عدد رسائل هذا العضو في الجروب</code>",
        parse_mode="HTML",
    )
