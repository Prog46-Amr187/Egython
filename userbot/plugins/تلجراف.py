...
©Egy : @Egython
  ...
  - Telegraph Media Commend: .تلجراف ميديا
  - Telegraph Text Commend: .تلجراف نص
"""
...
@icssbot.on(
    icss_cmd(
       pattern="تلجراف (ميديا|نص) ?(.*)"
    )
)
@icssbot.on(sudo_cmd(pattern="تلجراف(ميديا|نص) ?(.*)", allow_sudo=True))
@icssbot.on(
    icss_cmd(
       pattern="tg(m|t) ?(.*)"
    )
