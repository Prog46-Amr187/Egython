...
DEFAULTUSERBIO = DEFAULT_BIO or "واذكر ربك اذا نسيت - @Egython"
...
async def autopic(event):
    ...
    if gvarstatus("autopic") is not None and gvarstatus("autopic") == "true":
        return await edit_delete(event, f"**التغير التلقائي لـ بروفايلك بالفعل مفعل.**")
    addgvar("autopic", True)
    if input_str:
        addgvar("autopic_counter", input_str)
    await edit_delete(event, f"**تم تفعيل التغير التلقائي لـ بروفايلك.**")
    await autopicloop()
...
async def main(event):
    ...
    if gvarstatus("digitalpic") is not None and gvarstatus("digitalpic") == "true":
        return await edit_delete(event, f"**التـغير التـلقائـي لصورتك ممكن بالفعل .**")
    addgvar("digitalpic", True)
    await edit_delete(event, f"**تـم تفـعيل التـغير التـلقائـي لصورتك .**")
...
async def _(event):
    ...
    await edit_delete(event, "** تم تفعيل الـنبذة التلقائيه بنجاح .**")
    await autobio_loop()
...
async def _(event):  # sourcery no-metrics
        ...
        if gvarstatus("autopic") is not None and gvarstatus("autopic") == "true":
            ...
            if os.path.exists(autopic_path):
                ...
                try:
                    ...
                    return
            return await edit_delete(event, "** تم انهاء  البروفايل التلقائي الان .**")
        return await edit_delete(event, "** لم يتم تمكين  البروفايل التلقائي .**")
    ...
    if input_str == "الاسم تلقائي":
        if gvarstatus("autoname") is not None and gvarstatus("autoname") == "true":
            ...
            return await edit_delete(event, "**تم إيقاف لاسم التلقائي الآن .**")
    ...
    if input_str == "البايو تلقائي":
        if gvarstatus("autobio") is not None and gvarstatus("autobio") == "true":
            ...
            return await edit_delete(event, "** تم انهاء  البايو التلقائي الان .**")
        return await edit_delete(event, "** لم يتم تمكين  البايو التلقائي .**")
...
CMD_HELP.update(
  ...
  **◄ الامر **`.الاسم تلقائي`
•  **• الشرح **__لوضـع اسـم وقتـي لحسابـك يتغيـر تلقائيـاً كـل دقيقـه مـع الوقـت__
  **◄ الامر **`.البروفايل تلقائي`
•  **• الشرح **__لوضـع بروفايـل وقتـي يتغيـر تلقائيـاً مع حسابـك كل دقيقـه لشرح الامر اذهب الى https://t.me/Ralls_Thon/63 __
  **◄ الامر **`.البايو تلقائي`
•  **• الشرح **__لوضـع بايـو وقتـي يتغيـر تلقائـياً مع الوقـت كـل دقيقـه لشرح الامر اذهب الى https://t.me/Ralls_Thon/63 __
  **◄ الامر **`.انهاء + ( الاسم تلقائي/البروفايل تلقائي/البايو تلقائي ) `
•  **• الشرح **__لانهـاء الاوامـر الوقتيـه من حسابـك__
"""
    }
)
